...
var baseMaterial = new THREE.MeshLambertMaterial({ 
	...
	//___________________________________________ FAKE LIGHT
		...
		loader.load( ĂĽ.models.couch, function ( geometry ) {
		...
		var loader = new THREE.JSONLoader( true );
		loader.load( ĂĽ.models.monitor , function ( geometry ) {
      ...
      navigator.getUserMedia  = navigator.getUserMedia ||
		...
		loader.load( ĂĽ.models.tv, function ( geometry ) {
		...
		loader.load(ĂĽ.models.lamp, function ( geometry,material ) {
		...
		loader.load(ĂĽ.models.kid, function ( geometry) {
...
function clicker(obj){
  ...
  obj.parent.children.forEach(function(el)Â {
...
function onDocumentMouseDown( event ) {
	ĂĽ.mouse.x = ( event.clientX / window.innerWidth ) * 2 - 1;
	ĂĽ.mouse.y = - ( event.clientY / window.innerHeight ) * 2 + 1;	
	// find intersections
	var vector = new THREE.Vector3( ĂĽ.mouse.x, ĂĽ.mouse.y, 1 );
		vector.unproject( camera );
	var ray = new THREE.Raycaster( camera.position, vector.sub( camera.position ).normalize());
	var intersects = ray.intersectObjects( ĂĽ.targetList );
